...
export default async function DailyReportPage({ searchParams }: PageProps) {
  ...
  // ─── SVG Chart Logic (Rendering Grafik Murni Sisi Server) ────────────────
  const svgWidth = 800;
  const svgHeight = 280;
  ...
  // Bangun path SVG untuk Pemasukan (Income)
  let incomeLine = "";
  let incomeArea = "";
  data.forEach((d, i) => {
    const x = getX(i);
  ...
  // Bangun path SVG untuk Pengeluaran (Expense)
  let expenseLine = "";
  let expenseArea = "";
  data.forEach((d, i) => {
    const x = getX(i);
  ...
  return (
    <div className="w-full flex flex-col gap-4 py-2 pb-20 px-4 sm:px-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 py-2">
        <div className="max-w-xl">
          ...
          <p className="text-zinc-500 font-medium text-sm mt-3">
            Analisis data penjualan dan aktivitas finansial Anda secara
      ...
      {/* Main Chart (SVG Rendering Murni Sisi Server) */}
        ...
        <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between gap-4 mb-6 relative z-10">
          <div>
            ...
            <p className="text-xs text-zinc-400 font-medium">
              Perbandingan pemasukan dan pengeluaran (Server-rendered SVG)
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="w-full overflow-x-auto">
          <div className="min-w-[760px] h-[280px]">
            <svg
              ...
              role="img"
              aria-label="Grafik Kinerja Arus Kas Harian"
            >
              <title>Grafik Kinerja Arus Kas Harian</title>
              <defs>
              ...
              {data.map((d, i) => {
                ...
                // Format period tanggal misal "2026-06-07" -> "07 Jun"
